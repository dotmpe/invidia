...
module.exports =
	tables:
		...
		elements: (table) ->
			...
			table.uuid('expr_id')
				.references('id').inTable('exprs')
			table.uuid('schema_id')
				.references('id').inTable('schema')
		...
		exprs: (table) ->
			...
			table.enum('container_type', ['elements', 'attributes'])
			table.enum('type', ['group', 'choice', 'interleave', 'oneOrMore', 'zeroOrMore', 'optional'])
