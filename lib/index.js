...
var commands = {
	...
	'x-create-schema': function(ctx, callback) {
		...
		fn = ctx.argv.file
		ctx.readerDef = [ 'php-rootvar', 'dictionary', ctx.argv.map ];
		core.readFile(fn, ctx).then(function(data) {
			if (ctx.argv.path) {
				data = core.resolvePath(data, ctx.argv.path);
			}
			console.log(fn, reader, ctx.argv.path, data);
			core.printAny(data);
			callback();
		});
	}
...
var core = {
	...
	/* TODO: later. templates for schema instance and such */
	...
	resolvePath: function(data, path) {
