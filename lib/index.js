...
var core = {
	...
	/* Run processFile on all available files.
	 * Creates a subcontext with each glob pattern.
	 */
	runScans: function(scans, ctx) {
		/* processFile tasks, resolve to model lists */
		var tasks = [];
		...
		_.each(scans, function( pattern, i ) {
			...
			if (ctx.readFile) {
				...
				if (minimatch('./'+ctx.readFile, './'+sub.pathFormat)) {
					tasks.push(
	...
	readFile: function(ctx) {
		if (typeof ctx.argv['show-file'] !== "string") {
			callback("read-file: Missing filename argument");
		}
		core.readFile(ctx.argv['show-file'], ctx).then(function(data) {
