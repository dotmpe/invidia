...
var commands = {
	'scan': function(ctx) {
		var subCtx = ctx.getSub({ 
			groups: [],
			basedir: fs.existsSync(ctx.argv.scan) 
				? ctx.argv.scan : 'sugarcrm' }, ctx);
...
var core = {
	 ...
	 * run in sequence.
	 */
	runMain: function(extensions, config) {

		return new Promise(function(resolve, reject) {

			Promise.all(
				[ extensions, config ]
			).then(function(exts, config) {
				var 
